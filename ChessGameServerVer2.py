...
def Server(clientsocket):
    ...
    server = str(input("Server's turn: "))
    ...
    server = f'{len(server):<{HEADER_SIZE}}' + server
    clientsocket.send(bytes(server, "utf-8"))
...
def Client(clientsocket):
    ...
    while True:
        ...
        if len(client) - HEADER_SIZE == msglen:  
            new_msg = True
            break
    
    #Extract the message from client
    client = client[HEADER_SIZE:]
...
def main():
    ...
    if toPlay == "Y":
        ...
        while True: 
            ...
            server_status = Server(clientsocket)
            if server_status == FAILURE:
                break
            client_status = Client(clientsocket)
